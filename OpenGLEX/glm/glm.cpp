...
glmFindOrAddTexture(GLMmodel* model, const char* name)
{
	...
	__glmWarning("allocated texture %d (id=%d,width=%g,height=%g)",model->numtextures-1, model->textures[model->numtextures-1].id, width, height);
    free(filename);
...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    if (!file) {
        __glmFatalError( "glmReadMTL() failed: can't open material file \"%s\".",filename);
    }
    free(filename);
