...
void Controls::CheckPulse(char Move)
{
	...
	if(PulseID == HAT_LEFT || PulseID == HAT_RIGHT)
	{
		if(PulseCounter<(PULSEWIDTH/4))
			PulseCounter++;
		else 
		{
...
{
#ifdef WIN32
	if(Joy1->ButtonHeld(6))GripPulse++;
	if(GripPulse>=0 && GripPulse<=PulseLimitLow)
	{
		if(Joy1->ButtonHeld(6))GripMinipulse=true;
		else GripMinipulse = false;
	}
	
	if(GripPulse>PulseLimitLow && GripPulse<=PulseLimitHigh)GripMinipulse=false;
	if(GripPulse>PulseLimitHigh)GripPulse=0;
	...
	MovingGripperOpen = GetKeyHeld('S')  || Joy1->ButtonHeld(2) || Joy2->ButtonHeld(2) || GripMinipulse;
	MovingGripperClose = GetKeyHeld('D')  || Joy1->ButtonHeld(4) || Joy2->ButtonHeld(4);
#else
	if(Joy->ButtonHeld(6))GripPulse++;
	if(GripPulse>=0 && GripPulse<=PulseLimitLow)
	{
		if(Joy->ButtonHeld(6))GripMinipulse=true;
		else GripMinipulse = false;
	}
	...
	MovingGripperOpen = GetKeyHeld('S')  || Joy->ButtonHeld(2) || Joy->ButtonHeld(14) || GripMinipulse;
...
#ifdef WIN32
	if(GetKeyHeld('M') || Joy1->ButtonHeld(7) || Joy2->ButtonHeld(7))
		CamButton = 1;
	else CamButton = 0;
...
#else
	if(GetKeyHeld('M') || Joy->ButtonHeld(7))
		CamButton = 1;
	else CamButton = 0;
...
void Controls::UpdateSystemControls()
{
	...
	if(GetKeyNewpress('U') || Joy2->ButtonNewpress(9))
	{
		if(CloseLoopControlSystem)
			CloseLoopControlSystem = false;
		else 
			CloseLoopControlSystem = true;
	}
	...
	if(GetKeyNewpress('U') || Joy->ButtonNewpress(21))
	{
		if(CloseLoopControlSystem)
			CloseLoopControlSystem = false;
		else 
